...
class Simulator():
    ...
    def __init__(self, arg, n=None, json=None, file=None, bitorder='little'):
        ...
        if type(arg) == int:
            ...
            if self.restoreFromFile(arg):
                ...
                raise Exception(
                    "Wrong parameter string for Simulator, Pass either valid json string or path to a state file.")
        else:
            raise Exception(
                "Wrong parameter type for Simulator, Pass either integer, valid json string or path to a state file.")
        ...
        if o is not None:
            if o._bitOrder == self._bitOrder:
                if isinstance(o, Simulator):
                    ...
                    return np.array_equal(np.round(self._register, self._precision),
                                          np.round(o._register, self._precision))
            else:
    ...
    def toJson(self):
        ...
        return json.dumps(
            {'n': self._n, 'amp': amp, 'phase': phase, 'bitorder': self._bitOrder, 'precision': self._precision})
    ...
    def get_state_vector(self):
        # Return the state vector as a 1D array.
        return self._register.flatten()
